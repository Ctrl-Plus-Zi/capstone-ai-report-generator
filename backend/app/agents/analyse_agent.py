...
"""
    ...
    return textwrap.dedent(f"""
        ...
        2. **반드시 생성해야 할 블록**
           - 분석 요약 마크다운 블록 (맨 처음)
           - 데이터가 있는 경우 해당 차트/테이블
           - 결론 마크다운 블록 (맨 마지막)
        
        3. **차트 생성 시 주의**
           - labels와 values 배열 길이가 동일해야 함
           - values는 실제 숫자 (문자열 X)
           - description에 핵심 인사이트 포함
        ...
        # 시작
        위 데이터를 분석하고, 도구들을 호출하여 보고서 블록을 생성하세요.
        최소 3개 이상의 블록을 생성해야 합니다.
    """).strip()
...
def create_analyse_agent(tool_llm, summary_llm, toolkit):
    ...
    def analyse_agent_node(state):
        ...
        if hasattr(ai_response, "content") and ai_response.content:
            llm_analysis = ai_response.content.strip()
            if llm_analysis:
                analysis_findings += llm_analysis
        
        # 분석 요약이 없으면 기본 문구
        if not analysis_findings.strip():
            analysis_findings = f"{org_name}에 대한 데이터 분석이 완료되었습니다."
        ...
        return {
            "messages": messages,
            # 기존 호환용
            "analysis_outline": analysis_findings,
